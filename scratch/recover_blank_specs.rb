# scratch/recover_blank_specs.rb
# Run with: bin/rails runner scratch/recover_blank_specs.rb

require 'nokogiri'
require 'open-uri'
require 'fileutils'

puts "=== Blank Specifications & Images Recovery Session ==="

# Find products with blank specs
target_products = Product.where(product_detail: [nil, ''])
puts "Found #{target_products.count} products with blank specifications."

if target_products.empty?
  puts "🎉 All products already have specifications!"
  exit 0
end
...
puts "Mapped HTML product nodes for: #{product_nodes.keys.inspect}"
...
target_products.each do |product|
  ...
  puts "\n========================================="
  puts "🔄 Recovering: #{product.title} (SKU: #{cpn})"
  puts "========================================="
  ...
  puts "Fetching details from: #{detail_url}"
  ...
  retries.times do |attempt|
    begin
      ...
      if attempt < retries - 1
        ...
        puts "  ❌ All detail fetch attempts failed for CPN #{cpn}."
      end
    end
  end

  if detail_html.blank?
    puts "❌ Skipping specs and image recovery due to connection failure."
    next
  end
  ...
  if product_detail_html.blank?
    puts "❌ Scraped specifications are blank. Skipping update."
    next
  end
  ...
  if product.save
    puts "✅ Specs successfully saved in database!"
    ...
    if hd_image_urls.any?
      hd_image_urls.each_with_index do |img_url, img_idx|
        begin
          ...
          temp_filename = "dom_data_recovery_#{cpn}_#{img_idx + 1}#{ext}"
...
puts "\n=== Recovery Complete ==="
puts "Successfully recovered specifications and image packs for: #{success_count} / #{target_products.count} products."
puts "Remaining products with blank specs: #{Product.where(product_detail: [nil, '']).count}"
